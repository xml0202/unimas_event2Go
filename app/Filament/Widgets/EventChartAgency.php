...
class EventChartAgency extends LineChartWidget
{
    ...
    private function getEventsPerMonth($ongoing = true)
    {
        ...
        $months = collect(range(1, 12))->map(function($month) use ($now, $ongoing){
            ...
            $query = Event::whereBetween('start_datetime', [$startOfMonth, $endOfMonth])->where('user_id', auth()->user()->id);
    ...
    {
        return auth()->user()->hasRole("Agency");
    }
}
